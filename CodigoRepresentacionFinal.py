    ...
    try:
        ...
        while True:
            try:
                ...
                print(f"[ERROR] Ha ocurrido un error en las transformaciones: {e}")
    except ValueError:
...
def obtenerPuntos(cantidad_puntos):
    ...
    for i in range(cantidad_puntos):
        print(f"P{i+1}: [{puntos_paralelogramo[i][0]}, {puntos_paralelogramo[i][1]}]")
...
def sesgado(sx, sy):  # Matriz de sesgado de los puntos
    matriz_sesgado = np.array([[1, sx, 0], [sy, 1, 0], [0, 0, 1]])
...
def rotacion_paralelogramo_punto(cant_puntos, angulo, punto_rotacion):
    ...
    for p in range(cant_puntos):
        ...
        punto_rotado = np.array(rotacion_punto_centro(punto, angulo, punto_rotacion))
...
def mostrar_rotacion(cantidad_puntos):
    ...
    graficarRotacion(puntos_rotados, [0, 0])
...
def mostrar_rotacion_desde_punto(cantidad_puntos):
    ...
    graficarRotacion(puntos_rotados, punto_rotacion)
...
def mostrar_traslacion(cantidad_puntos):
    ...
    puntos_trasladados = traslacion_paralelogramo(cantidad_puntos, trasl_x, tresl_y)
    print("\nPuntos después de la traslación:")
    # Paso 5: Imprime los puntos trasladados
    imprimir_matriz(cantidad_puntos, puntos_trasladados)

    graficarTransformacion(puntos_trasladados, "Traslacion")
...
def mostrar_sesgado(cantidad_puntos):
    ...
    graficarTransformacion(puntos_sesgados, "Sesgado")
...
def ordenar_puntos_en_sentido_horario(puntos):
    ...
    return np.array(
        sorted(
            puntos, key=lambda p: np.arctan2(p[1] - centroide[1], p[0] - centroide[0])
        )
    )
...
def graficarTransformacion(puntos_transformados, tipo_transformacion):
    ...
    plot_paralelogramo(
        puntos_transformados, "purple", f"Paralelogramo {tipo_transformacion}"
    )
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title(f"Paralelogramos Original y {tipo_transformacion}")
    plt.grid(True)
    plt.show()

def graficarRotacion(puntos_rotados, punto_rotacion):
    # Paso 1: Ordena los puntos en sentido horario para conectar el 
    # punto de rotación con el primer vértice
    puntos_ord = ordenar_puntos_en_sentido_horario(puntos_paralelogramo)
    puntos_rotados_ord = ordenar_puntos_en_sentido_horario(puntos_rotados)
    # Paso 2: Grafica el paralelogramo original en color deeppink
    plot_paralelogramo(puntos_paralelogramo, "deeppink", "Paralelogramo Original")
    # Paso 3: Grafica el paralelogramo rotado en color purple
    plot_paralelogramo(puntos_rotados, "purple", "Paralelogramo Rotado")
    # Paso 4: Dibuja un punto negro para representar el eje de rotación
    plt.scatter(
        punto_rotacion[0], punto_rotacion[1], color="black", label="Eje de Rotación"
    )
    plt.axis("equal")
    plt.legend()
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title("Paralelogramos Original y Rotado")
    plt.grid(True)
    plt.show()
